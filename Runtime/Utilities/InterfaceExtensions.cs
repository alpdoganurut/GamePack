...
namespace GamePack.Utilities
{
    public static class InterfaceExtensions
    {
        ...
        /// <returns></returns>
        public static T[] GetAllInterfaces<T>()
        {
            if (!typeof(T).IsInterface) throw new SystemException("Specified type is not an interface!");
            var mObjs = Resources.FindObjectsOfTypeAll<MonoBehaviour>();
    
            var interfaces = (from a in mObjs where a.GetType().GetInterfaces().Any(k => k == typeof(T)) select (T)(object)a).ToArray();
            return interfaces;
        }
